        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">المواصفات</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
    ...
    <section class="content">
      ...
      <div class="card">
        <div class="card-header">
          <h3 class="card-title">المواصفات</h3>

          <div class="card-tools">
          <a class="btn btn-info btn-sm" href="{{url('admin/property/create')}}">
                              <i class="fas fa-plus">
                              </i> جديد</a>
        ...
        <table id="example1" class="table table-bordered table-striped table-hover">
                ...
                <tr>
                    <th scope="row">{{ ++$i }}</th>
                  <td>{{$item->name}}</td>
                  <td>{{ $item->propertydep->name}}</td>
                  <td>{{ $item->is_multivalue}}</td>                  
                  <td>{{ $item->is_active }}</td>
                  <td>    <a class="btn btn-info btn-sm" href="{{url('admin/property/edit', $item->id)}}">
                              <i class="fas fa-pencil-alt">
                              </i>
                              تعديل
                          </a>
                          <form action="{{url('admin/property/destroy', $item->id)}}" method="POST" class="d-inline">
                            @csrf
                            @method('DELETE')
